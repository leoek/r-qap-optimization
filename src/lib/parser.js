...
export const createParser = ({ basePath = ".", fileExtension = "", parseFn = id => id }) => {
  const parseFile = (options = {}) => {
    const { path, name = "default" } = options;
    const fullPath = path ? resolvePath(basePath, path) : resolvePath(basePath, "problems", `${name}${fileExtension ? "." : ""}${fileExtension}`);
